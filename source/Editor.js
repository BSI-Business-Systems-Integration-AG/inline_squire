...
proto._getHTML = function () {
    ...
    // IE always adds ';' at the end of a style attribute, other browser don't. The server-side
    // transformation also removes the trailing semicolon. To prevent a "validation ping-pong"
    // between UI and model, we try to trip trailing semicolons from the style attribute here.
    // (We know that this regexp does not work in all possible cases. However, if the user injects
    // invalid HTML, we don't care and let the server strip it from the HTML. The regexp works
    // fine for content generated by the rich text field.)
    html = html.replace(/(<[^>]+\sstyle\s*=\s*(["']).*?);(\s*\2[^>]*>)/g, '$1$3');
    return html;
	// <CUSTOMIZED>
};
